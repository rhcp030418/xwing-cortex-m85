...
#define X4_XOF_BLOCKS 3u

static void xof_batch3(xof_state states[4],
                       unsigned char bufs[4][X4_XOF_BLOCKS * XOF_BLOCKBYTES + 2u],
                       unsigned char row, const unsigned char *seed, int transposed)
{
  ...
  for (unsigned int j = 0; j < 4; j++) {
    ...
    bufs[j][X4_XOF_BLOCKS * XOF_BLOCKBYTES] = 0;
    bufs[j][X4_XOF_BLOCKS * XOF_BLOCKBYTES + 1u] = 0;
  }
}
                              ...
                              unsigned char i, const unsigned char *seed, int transposed)
{
  ...
  matacc_asm_cache_16_32_x4(r_tmp, b->vec[0].coeffs, c, bufs[0], zetas,
                            &states[0], b_prime->vec[0].coeffs);
  matacc_asm_cache_32_32_x4(r_tmp, b->vec[1].coeffs, c, bufs[1], zetas,
                            &states[1], b_prime->vec[1].coeffs);
  matacc_asm_cache_32_16_x4(r->coeffs, b->vec[2].coeffs, c, bufs[2], zetas,
                            ...
                            unsigned char i, const unsigned char *seed, int transposed)
{
  ...
  xof_batch3(states, bufs, i, seed, transposed);
  matacc_asm_opt_16_32_x4(r_tmp, b->vec[0].coeffs, c, bufs[0], &states[0],
                          b_prime->vec[0].coeffs);
  matacc_asm_opt_32_32_x4(r_tmp, b->vec[1].coeffs, c, bufs[1], &states[1],
                          b_prime->vec[1].coeffs);
  matacc_asm_opt_32_16_x4(r->coeffs, b->vec[2].coeffs, c, bufs[2], &states[2],
