"""Flash expCS, summarize slots 862..867 (solver-rescheduled phase 0: F/U/Us)."""
...
BASE = 862
CALLS = 24        # 1-job 융합 구간에서 phase-0 조각 호출 횟수
...
def summarize(log, fails, reps, elf, srec, rb, rbn) -> dict:
    ...
    kv = _parse_kv(log, "expCS mismatch=")
    mm = int(kv.get("", -1)) if kv else -1
    for line in log.splitlines():
        if line.startswith("expCS mismatch="):
...
def print_effects(p):
    print("")
    print("=== expCS — phase-0 솔버 재스케줄 (F / U 지퍼 / U_s 솔버) ===")
...
def self_test():
    assert SLOT_N == 870 and FIRST_SLOT == BASE == 862 and BATCH == 1
    print("self-test OK")
...
def main():
    ...
    print("EXPCS_JSON " + json.dumps(payload, ensure_ascii=False, sort_keys=True))
