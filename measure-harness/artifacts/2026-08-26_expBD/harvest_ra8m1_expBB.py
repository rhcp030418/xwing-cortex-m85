"""Flash expBB, summarize slots 548..552 (fe25519_sqr reschedule A/B)."""
...
def summarize(log, fails, reps, elf, srec, rb, rbn) -> dict:
    ...
    null = _med(reps, 4) / BATCH
    a1 = _med(reps, 0) / BATCH; b1 = _med(reps, 1) / BATCH
    b2 = _med(reps, 2) / BATCH; a2 = _med(reps, 3) / BATCH
    p1 = 100.0 * (a1 - b1) / a1
    p2 = 100.0 * (a2 - b2) / a2
    # BD (mul) 슬롯 566..569 = A B B A
    ma1 = _med(reps, 566 - BASE) / BATCH; mb1 = _med(reps, 567 - BASE) / BATCH
    mb2 = _med(reps, 568 - BASE) / BATCH; ma2 = _med(reps, 569 - BASE) / BATCH
    mp1 = 100.0 * (ma1 - mb1) / ma1
    mp2 = 100.0 * (ma2 - mb2) / ma2
    mul_cons = min(mp1, mp2)

    kv = _parse_kv(log, "expBB correctness:")
    mm = int(kv.get("mm", -1)); chk = int(kv.get("checks", -1))
    mul_mm = int(kv.get("mul-mm", -1))
    kat = "KAT ALL PASS" in log
    ok = (fails == 0) and (mm == 0) and (mul_mm == 0) and kat
    cons = min(p1, p2)
    return {
        "experiment": "BB",
        "harness_fails": fails, "mismatch": mm, "checks": chk,
        "kat_all_pass": kat, "correctness_pass": ok,
        "A_orig": {"AB": a1, "BA": a2, "median": (a1 + a2) / 2.0},
        "B_resched": {"AB": b1, "BA": b2, "median": (b1 + b2) / 2.0},
        "null": null,
        "saving_pct_AB": p1, "saving_pct_BA": p2, "saving_pct_conservative": cons,
        "gate_5pct": cons >= GATE_PCT,
        "mul_mismatch": mul_mm,
        "mul_A": {"AB": ma1, "BA": ma2, "median": (ma1 + ma2) / 2.0},
        "mul_B": {"AB": mb1, "BA": mb2, "median": (mb1 + mb2) / 2.0},
        "mul_saving_pct_conservative": mul_cons,
        "mul_gate_5pct": mul_cons >= GATE_PCT,
        "elf_sha256": sha256(elf), "srec_sha256": sha256(srec),
        "code_flash_readback_sha256": rb, "code_flash_readback_bytes": rbn,
        "gate_pass": ok and (cons >= GATE_PCT),
    }


def print_effects(p):
    print("")
    print("=== expBB — fe25519_sqr 재스케줄 A/B ===")
    print("정확성: %s (harness_fails=%s, mismatch=%s, checks=%s, KAT=%s)"
          % ("PASS" if p["correctness_pass"] else "FAIL", p["harness_fails"],
             p["mismatch"], p["checks"], p["kat_all_pass"]))
    print("  A 원본      AB %8.2f  BA %8.2f  (median %.2f)"
          % (p["A_orig"]["AB"], p["A_orig"]["BA"], p["A_orig"]["median"]))
    print("  B 재스케줄  AB %8.2f  BA %8.2f  (median %.2f)"
          % (p["B_resched"]["AB"], p["B_resched"]["BA"], p["B_resched"]["median"]))
    print("  절감 AB %+.2f%%  BA %+.2f%%  -> conservative %+.2f%%"
          % (p["saving_pct_AB"], p["saving_pct_BA"], p["saving_pct_conservative"]))
    print("  사전등록 gate (>=%.0f%%): %s"
          % (GATE_PCT, "PASS" if p["gate_5pct"] else "FAIL"))
    print("  (참고) sqr 모델 예측 85 -> 66,  null 슬롯 %.2f" % p["null"])
    print("")
    print("  --- BD: fe25519_mul (mismatch=%s)" % p["mul_mismatch"])
    print("  A 원본      AB %8.2f  BA %8.2f  (median %.2f)"
          % (p["mul_A"]["AB"], p["mul_A"]["BA"], p["mul_A"]["median"]))
    print("  B 재스케줄  AB %8.2f  BA %8.2f  (median %.2f)"
          % (p["mul_B"]["AB"], p["mul_B"]["BA"], p["mul_B"]["median"]))
    print("  절감 conservative %+.2f%%  -> gate(>=5%%) %s"
          % (p["mul_saving_pct_conservative"],
             "PASS" if p["mul_gate_5pct"] else "FAIL"))


def self_test():
    assert SLOT_N == 570 and FIRST_SLOT == BASE == 548
    print("self-test OK")
...
def main():
    ...
    print("EXPBB_JSON " + json.dumps(payload, ensure_ascii=False, sort_keys=True))
