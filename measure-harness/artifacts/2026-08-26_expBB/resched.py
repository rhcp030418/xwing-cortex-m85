    ...
    while k < len(order):
        ...
        while k < len(order) and issued < 2 and not blocked:
            ...
            if a.is_store:
                if issued:
                    break
                finish[i] = cyc; k += 1; issued = 1; blocked = True; continue
            if a.is_mul:
                if mul_used:
                    break
                mul_used = True
...
def main(path, out):
    ...
    # 고정점(push/pop/sp 조작) 사이의 구간만 재정렬한다
    fixed = [k for k, ln in enumerate(src)
             if re.match(r"\s*(push|pop|sub\s+sp|add\s+sp)\b", ln)]
    blocks, prev = [], 0
