"""Flash and summarize expCJ draft-10 cumulative same-ELF A/B."""
...
def self_test():
    fake = [0] * (SLOT_N * REP_N)
    for slot in range(570, 586):
        fake[slot * REP_N:(slot + 1) * REP_N] = [slot] * REP_N
    assert SLOT_N == 775 and FIRST_SLOT == BASE == 570
    assert median(fake[570 * REP_N:571 * REP_N]) == 570
    print("self-test OK")


def summarize_cj(log, fails, reps, elf, srec, rb, rbn):
    ops = ("keygen", "encaps", "decaps-warm", "decaps-cold")
    out = {}
    stable = True
    for oi, op in enumerate(ops):
        a1 = median(reps[(570 + oi) * REP_N:(571 + oi) * REP_N])
        b1 = median(reps[(574 + oi) * REP_N:(575 + oi) * REP_N])
        b2 = median(reps[(578 + oi) * REP_N:(579 + oi) * REP_N])
        a2 = median(reps[(582 + oi) * REP_N:(583 + oi) * REP_N])
        p1 = 100.0 * (a1 - b1) / a1
        p2 = 100.0 * (a2 - b2) / a2
        drift_a = 200.0 * abs(a1 - a2) / (a1 + a2)
        drift_b = 200.0 * abs(b1 - b2) / (b1 + b2)
        stable &= drift_a <= 0.3 and drift_b <= 0.3
        out[op] = {
            "A1": a1, "B1": b1, "B2": b2, "A2": a2,
            "saving_pct_pair1": p1, "saving_pct_pair2": p2,
            "saving_pct_conservative": min(p1, p2),
            "drift_A_pct": drift_a, "drift_B_pct": drift_b,
        }

    checks = {
        "bytes_mm": _kvp(log, "expCJ equiv:", "bytes-mm="),
        "smoke": _kvp(log, "expCJ equiv:", "smoke="),
        "reject_mm": _kvp(log, "expCJ equiv:", "reject-mm="),
        "coldwarm_mm": _kvp(log, "expCJ equiv:", "coldwarm-mm="),
        "timing_mm": _kvp(log, "expCJ timing-mm=", "timing-mm="),
        "stack_fails": _kvp(log, "expCJ timing-mm=", "stack-fails="),
        "stack_used": _kvp(log, "expCJ timing-mm=", "stack-used="),
    }
    correctness = fails == 0 and all(checks[k] == 0 for k in (
        "bytes_mm", "smoke", "reject_mm", "coldwarm_mm", "timing_mm", "stack_fails"
    )) and "KAT ALL PASS" in log
    return {
        "experiment": "CJ",
        "correctness_pass": correctness,
        "within_run_stability_pass": bool(stable),
        "valid": bool(correctness and stable),
        "harness_fails": fails,
        "checks": checks,
        "ops": out,
        "elf_sha256": sha256(elf),
        "srec_sha256": sha256(srec),
        "code_flash_readback_sha256": rb,
        "code_flash_readback_bytes": rbn,
    }


def print_cj(payload):
    print("\n=== expCJ — draft-10 최종 누적 동일-ELF A/B ===")
    print("correctness=%s stability=%s harness_fails=%d checks=%s" % (
        "PASS" if payload["correctness_pass"] else "FAIL",
        "PASS" if payload["within_run_stability_pass"] else "FAIL",
        payload["harness_fails"], payload["checks"],
    ))
    print("%-13s %10s %10s %10s %10s %10s %10s" % (
        "operation", "A1", "B1", "B2", "A2", "save1", "save2"
    ))
    for op, row in payload["ops"].items():
        print("%-13s %10d %10d %10d %10d %9.3f%% %9.3f%%" % (
            op, row["A1"], row["B1"], row["B2"], row["A2"],
            row["saving_pct_pair1"], row["saving_pct_pair2"],
        ))
    print("EXPCJ_JSON " + json.dumps(payload, ensure_ascii=False, sort_keys=True))
...
def main():
    ...
    print(log)
    payload = summarize_cj(log, fails, reps, args.elf, args.srec, rb, rbn)
    print_cj(payload)
    return 0 if payload["valid"] else 2

    # ---- expCG (게스트 재물질화 할당: 실물 X25519 sqr 호스트) ----
    B16g = 16.0
    og = 764 - FIRST_SLOT
    Pg = _med(reps, og) / B16g
    # ABBA: A og+1/og+8, B og+2/og+7, C og+3/og+6, D(호이스팅) og+4/og+5
    def cell(i, j):
        return ((_med(reps, og + i) + _med(reps, og + j)) / 2.0 / B16g) - Pg
    Ag, Bg = cell(1, 10), cell(2, 9)
    Cg, Dg, Eg = cell(3, 8), cell(4, 7), cell(5, 6)
    hide = (Ag + Bg - Cg) / Bg * 100.0 if Bg else 0.0
    print()
    print("=== expCG — 게스트 재물질화 할당 (호스트 = fe25519_sqr_bb 원본 그대로) ===")
    print("  정확성 host-mm=%s  guest-mm=%s  shl-mm=%s  checks=%s"
          % (_kvp(log, "expCG ", "host-mm="), _kvp(log, "expCG ", "guest-mm="),
             _kvp(log, "expCG ", "shl-mm="), _kvp(log, "expCG ", "checks=")))
    print("  프롤로그 P = %.2f cyc (A/B/C 에서 뺀 값)" % Pg)
    print()
    print("  %-22s %10s" % ("A (NTT layer67 x4)", "%.2f" % Ag))
    print("  %-22s %10s" % ("B (fe25519_sqr)", "%.2f" % Bg))
    print("  %-22s %10s" % ("C (병합)", "%.2f" % Cg))
    print("  %-22s %10s" % ("D (병합+호이스팅)", "%.2f" % Dg))
    print("  %-22s %10s" % ("E (병합+ldr 풀)", "%.2f" % Eg))
    print("  %-22s %10s" % ("A + B (순차)", "%.2f" % (Ag + Bg)))
    print("  %-22s %10s" % ("C - A (호스트 누출)", "%.2f" % (Cg - Ag)))
    print()
    for nm, i, j in (("A", 1, 10), ("B", 2, 9), ("C", 3, 8), ("D", 4, 7), ("E", 5, 6)):
        print("  ABBA %s: %.2f / %.2f" % (nm, _med(reps, og + i) / B16g - Pg,
                                          _med(reps, og + j) / B16g - Pg))
    print()
    # expCE 법칙 검증: 스칼라 169개(호스트 94 + 재물질화 75) x 0.34
    n_scalar, n_remat = 169, 75
    pred = 0.34 * n_scalar
    print("  expCE 법칙 예측: 0.34 x %d(스칼라) = %.1f cyc,  실측 C-A = %.2f cyc"
          % (n_scalar, pred, Cg - Ag))
    print("  실측 누출률 k = (C-A)/스칼라수 = %.3f cyc/명령 (expCE: 0.34)"
          % ((Cg - Ag) / n_scalar))
    print()
    hideD = (Ag + Bg - Dg) / Bg * 100.0 if Bg else 0.0
    print("  === expCH: 재물질화 호이스팅 (인접 재물질화 56 -> 28) ===")
    print("  D - C = %+.2f cyc   (M1 'RAW 스톨' 이면 음수, M2 'movw 듀얼이슈 불가' 면 0)" % (Dg - Cg))
    print("  D 의 재물질화 단가 = (D-A-0.34x94)/75 = %.3f cyc/명령  (C 는 %.3f)"
          % ((Dg - Ag - 0.34 * 94) / 75.0, (Cg - Ag - 0.34 * 94) / 75.0))
    print("  D 은닉률 (A+B-D)/B = %.1f%%" % hideD)
    print("  CH-1  은닉률 >= 50%% : %s" % ("PASS" if hideD >= 50.0 else "FAIL"))
    print("  CH-2  D < A + B     : %s" % ("PASS" if Dg < Ag + Bg else "FAIL"))
    print()
    hideE = (Ag + Bg - Eg) / Bg * 100.0 if Bg else 0.0
    print("  === expCI: movw 재물질화 -> 스택 상수 풀의 ldr (75 -> 55 명령) ===")
    print("  E - C = %+.2f cyc   (풀 셋업 9명령이 E 에만 들어 있다)" % (Eg - Cg))
    print("  E 의 재물질화 단가 = (E-A-0.34x94)/55 = %.3f cyc/명령" % ((Eg - Ag - 0.34 * 94) / 55.0))
    print("  E 은닉률 (A+B-E)/B = %.1f%%" % hideE)
    print("  CI-1  은닉률 >= 50%% : %s" % ("PASS" if hideE >= 50.0 else "FAIL"))
    print("  CI-2  E < A + B     : %s" % ("PASS" if Eg < Ag + Bg else "FAIL"))
    print()
    print("  은닉률 (A+B-C)/B = %.1f%%" % hide)
    print("  CG-1  은닉률 >= 50%% : %s" % ("PASS" if hide >= 50.0 else "FAIL"))
    print("  CG-2  C < A + B     : %s" % ("PASS" if Cg < Ag + Bg else "FAIL"))
    print("EXPCG_JSON " + json.dumps({"P": Pg, "A": Ag, "B": Bg, "C": Cg,
                                      "hide_pct": hide, "n_scalar": n_scalar,
                                      "D": Dg, "hide_pct_D": hideD, "E": Eg, "hide_pct_E": hideE,
                                      "n_remat": n_remat}, sort_keys=True))
    print()
    ...
    B16e = 16.0
    oe = 753 - 570
    Ae = _med(reps, oe) / B16e
    rr = [0.0, 7.7, 16.0, 23.7, 32.1]
    print()
    print("=== expCE — 누출률 법칙 k(메모리 비율) ===")
    print("  정확성 ntt-mm=%s" % _kvp(log, "expCE correct:", "ntt-mm="))
    print("  A (NTT 만) = %.2f cyc" % Ae)
    print()
    print("  %8s %9s %9s %9s %8s %8s" % ("메모리%", "B", "C", "C-A", "k", "은닉률"))
    law = {}
    for i, r in enumerate(rr):
        Bi = _med(reps, oe + 1 + 2 * i) / B16e
        Ci = _med(reps, oe + 2 + 2 * i) / B16e
        k = (Ci - Ae) / Bi if Bi else 0.0
        law["%.1f" % r] = {"B": Bi, "C": Ci, "k": k}
        print("  %8.1f %9.2f %9.2f %9.2f %8.3f %7.1f%%"
              % (r, Bi, Ci, Ci - Ae, k, 100.0 * (1.0 - k)))
    k0 = law["0.0"]["k"]
    print()
    print("  CE-1  k(0) <= 0.10 : k(0) = %.3f = %s" % (k0, "PASS" if k0 <= 0.10 else "FAIL"))
    print("EXPCE_JSON " + json.dumps({"A": Ae, "law": law}, sort_keys=True))
    print()
    # ---- expCD (은닉 용량 곡선) ----
    B16d = 16.0
    o = 736 - 570
    Aa = _med(reps, o) / B16d
    print()
    print("=== expCD — 은닉 용량 곡선 (intt layer67 x4 = A, Keccak lane 1..8) ===")
    print("  정확성 ntt-mm=%s  kec-mm=%s"
          % (_kvp(log, "expCD correct:", "ntt-mm="),
             _kvp(log, "expCD correct:", "kec-mm=")))
    print("  A (NTT 만) = %.2f cyc,   A x 1.02 = %.2f" % (Aa, Aa * 1.02))
    print()
    print("  %4s %9s %9s %9s %9s %8s %6s" %
          ("lane", "B(n)", "C(n)", "A+B", "C-A", "은닉률", "공짜?"))
    F = 0.0
    best_n = 0
    curve = {}
    for n in range(1, 9):
        Bn = _med(reps, o + 1 + 2 * (n - 1)) / B16d
        Cn = _med(reps, o + 2 + 2 * (n - 1)) / B16d
        hide = (Aa + Bn - Cn) / Bn * 100.0 if Bn else 0.0
        free = Cn <= Aa * 1.02
        if free:
            F = Bn; best_n = n
        curve[n] = {"B": Bn, "C": Cn, "hide_pct": hide, "free": free}
        print("  %4d %9.2f %9.2f %9.2f %9.2f %7.1f%% %6s"
              % (n, Bn, Cn, Aa + Bn, Cn - Aa, hide, "O" if free else "-"))
    print()
    print("  공짜 용량 F = %.2f cyc (lane %d)   =  A 의 %.1f%%" % (F, best_n, 100.0 * F / Aa))
    print("  CD-1  F >= 0.60 x A (%.2f) = %s" % (0.60 * Aa, "PASS" if F >= 0.60 * Aa else "FAIL"))
    print("EXPCD_JSON " + json.dumps({"A": Aa, "F": F, "best_lane": best_n,
                                      "curve": curve}, sort_keys=True))
    print()
    # ---- expCC (교차 종류별 겹침) ----
    B64x = 64.0
    off = 724 - 570
