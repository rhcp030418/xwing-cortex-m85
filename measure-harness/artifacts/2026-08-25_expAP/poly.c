...
*************************************************/
static void ap_inner_poly_compress(unsigned char *r, const poly *a)
{
  if (mlkem_c9_mode) {               /* 실험 AJ: MVE 경로 */
      mve_poly_compress(r, a);
      return;
  }
  unsigned int i,j;
  int16_t u;
...
**************************************************/
static void ap_inner_poly_decompress(poly *r, const unsigned char *a)
{
  if (mlkem_c9_mode) {               /* 실험 AJ: MVE 경로 */
      mve_poly_decompress(r, a);
      return;
  }
  int i;
#if (KYBER_POLYCOMPRESSEDBYTES == 128)
...
**************************************************/
static void ap_inner_poly_packcompress(unsigned char *r, poly *a, int i) {
    if (mlkem_c9_mode) {             /* 실험 AJ: MVE 경로 */
        mve_poly_packcompress(r, a, i);
        return;
    }
    int j, k;
    uint64_t d0;
...
**************************************************/
static void ap_inner_poly_unpackdecompress(poly *r, const unsigned char *a, int i) {
  if (mlkem_c9_mode) {               /* 실험 AJ: MVE 경로 */
      mve_poly_unpackdecompress(r, a, i);
      return;
  }
  int j;
#if (KYBER_POLYVECCOMPRESSEDBYTES == (KYBER_K * 352))
...
int cmp_poly_compress(const unsigned char *r, poly *a) {
    if (mlkem_c9_mode) {             /* 실험 AJ: MVE 경로 */
        return mve_cmp_poly_compress(r, a);
    }
    unsigned char rc = 0;
    int16_t u;
    uint32_t d0;
...
int cmp_poly_packcompress(const unsigned char *r, poly *a, int i) {
    if (mlkem_c9_mode) {             /* 실험 AJ: MVE 경로 */
        return mve_cmp_poly_packcompress(r, a, i);
    }
    unsigned char rc = 0;
    int j, k;
    uint64_t d0;
...
*              - const poly *a:    pointer to input polynomial
**************************************************/
static void ap_inner_poly_tobytes(unsigned char *r, poly *a) {
    if (mlkem_c9_mode) {             /* 실험 AJ: MVE 경로 */
        mve_poly_tobytes(r, a);
        return;
    }
    int i;
    uint16_t t0, t1;
...
**************************************************/
static void ap_inner_poly_frombytes(poly *r, const unsigned char *a) {
    if (mlkem_c9_mode) {             /* 실험 AJ: MVE 경로 */
        mve_poly_frombytes(r, a);
        return;
    }
    int i;
...
static void ap_inner_poly_frombytes_mul_16_32(int32_t *r_tmp, const poly *b, const unsigned char *a) {
...
static void ap_inner_poly_frombytes_mul_32_32(int32_t *r_tmp, const poly *b, const unsigned char *a) {
...
static void ap_inner_poly_frombytes_mul_32_16(poly *r, const poly* b, const unsigned char *a, const int32_t *r_tmp) {
...
**************************************************/
static void ap_inner_poly_noise(poly *r, const unsigned char *seed, unsigned char nonce, int add) {
...
static void ap_inner_poly_basemul_opt_16_32(int32_t *r_tmp, const poly *a, const poly *b, const poly *a_prime) {
...
static void ap_inner_poly_basemul_acc_opt_32_32(int32_t *r_tmp, const poly *a, const poly *b, const poly *a_prime) {
...
static void ap_inner_poly_basemul_acc_opt_32_16(poly *r, const poly *a, const poly *b, const poly *a_prime, const int32_t * r_tmp) {
...
**************************************************/
static void ap_inner_poly_ntt(poly *r) {
    ntt(r->coeffs);
}
...
**************************************************/
static void ap_inner_poly_invntt(poly *r) {
    invntt(r->coeffs);
}
...
**************************************************/
static void ap_inner_poly_fromplant(poly *r) {
...
* Arguments:   - poly *r:       pointer to input/output polynomial
**************************************************/
static void ap_inner_poly_reduce(poly *r) {
...
**************************************************/
static void ap_inner_poly_add(poly *r, const poly *a, const poly *b) {
...
*            - const poly *b: pointer to second input polynomial
**************************************************/
static void ap_inner_poly_sub(poly *r, const poly *a, const poly *b) {
...
**************************************************/
static void ap_inner_poly_frommsg(poly *r, const uint8_t msg[KYBER_INDCPA_MSGBYTES])
{
  if (mlkem_c9_mode) {               /* 실험 AJ: MVE 경로 */
      mve_poly_frommsg(r, msg);
      return;
  }
  unsigned int i,j;
...
*              - const poly *a:      pointer to input polynomial
**************************************************/
static void ap_inner_poly_tomsg(unsigned char msg[KYBER_SYMBYTES], poly *a) {
    if (mlkem_c9_mode) {             /* 실험 AJ: MVE 경로 */
        mve_poly_tomsg(msg, a);
        return;
    }
    uint32_t t;
    int i, j;
...
{
    AP_WRAP(AP_B_NTT, ap_inner_poly_ntt(r));
}

void poly_invntt(poly *r)
{
    AP_WRAP(AP_B_INVNTT, ap_inner_poly_invntt(r));
}

void poly_noise(poly *r, const unsigned char *seed, unsigned char nonce, int add)
{
    AP_WRAP(AP_B_NOISE, ap_inner_poly_noise(r, seed, nonce, add));
}

void poly_compress(unsigned char *r, const poly *a)
{
    AP_WRAP(AP_B_PACK, ap_inner_poly_compress(r, a));
}

void poly_decompress(poly *r, const unsigned char *a)
{
    AP_WRAP(AP_B_PACK, ap_inner_poly_decompress(r, a));
}

void poly_packcompress(unsigned char *r, poly *a, int i)
{
    AP_WRAP(AP_B_PACK, ap_inner_poly_packcompress(r, a, i));
}

void poly_unpackdecompress(poly *r, const unsigned char *a, int i)
{
    AP_WRAP(AP_B_PACK, ap_inner_poly_unpackdecompress(r, a, i));
}

void poly_tobytes(unsigned char *r, poly *a)
{
    AP_WRAP(AP_B_PACK, ap_inner_poly_tobytes(r, a));
}

void poly_frombytes(poly *r, const unsigned char *a)
{
    AP_WRAP(AP_B_PACK, ap_inner_poly_frombytes(r, a));
}

void poly_frommsg(poly *r, const uint8_t msg[KYBER_INDCPA_MSGBYTES])
{
    AP_WRAP(AP_B_PACK, ap_inner_poly_frommsg(r, msg));
}

void poly_tomsg(unsigned char msg[KYBER_SYMBYTES], poly *a)
{
    AP_WRAP(AP_B_PACK, ap_inner_poly_tomsg(msg, a));
}

void poly_reduce(poly *r)
{
    AP_WRAP(AP_B_ARITH, ap_inner_poly_reduce(r));
}

void poly_fromplant(poly *r)
{
    AP_WRAP(AP_B_ARITH, ap_inner_poly_fromplant(r));
}

void poly_add(poly *r, const poly *a, const poly *b)
{
    AP_WRAP(AP_B_ARITH, ap_inner_poly_add(r, a, b));
}

void poly_sub(poly *r, const poly *a, const poly *b)
{
    AP_WRAP(AP_B_ARITH, ap_inner_poly_sub(r, a, b));
}

void poly_basemul_opt_16_32(int32_t *r_tmp, const poly *a, const poly *b, const poly *a_prime)
{
    AP_WRAP(AP_B_BASEMUL, ap_inner_poly_basemul_opt_16_32(r_tmp, a, b, a_prime));
}

void poly_basemul_acc_opt_32_32(int32_t *r_tmp, const poly *a, const poly *b, const poly *a_prime)
{
    AP_WRAP(AP_B_BASEMUL, ap_inner_poly_basemul_acc_opt_32_32(r_tmp, a, b, a_prime));
}

void poly_basemul_acc_opt_32_16(poly *r, const poly *a, const poly *b, const poly *a_prime, const int32_t * r_tmp)
{
    AP_WRAP(AP_B_BASEMUL, ap_inner_poly_basemul_acc_opt_32_16(r, a, b, a_prime, r_tmp));
}

void poly_frombytes_mul_16_32(int32_t *r_tmp, const poly *b, const unsigned char *a)
{
    AP_WRAP(AP_B_BASEMUL, ap_inner_poly_frombytes_mul_16_32(r_tmp, b, a));
}

void poly_frombytes_mul_32_32(int32_t *r_tmp, const poly *b, const unsigned char *a)
{
    AP_WRAP(AP_B_BASEMUL, ap_inner_poly_frombytes_mul_32_32(r_tmp, b, a));
}

void poly_frombytes_mul_32_16(poly *r, const poly* b, const unsigned char *a, const int32_t *r_tmp)
{
    AP_WRAP(AP_B_BASEMUL, ap_inner_poly_frombytes_mul_32_16(r, b, a, r_tmp));
}
