"""Flash expAR, collect the RAM log, and summarize slots 435..446 (X25519 field-op attribution)."""
...
# within op: 0 total_off, 1 total_on, 2 mul(cyc/cnt), 3 sqr(cyc/cnt)

# same-ELF expAN slots: X25519 구간을 이 빌드에서 직접 읽는다
AN_BASE = 362
AN = {  # (upper_ckpt_offset, lower_ckpt_offset or None) -> X25519 구간
    "keygen": (2, 1),          # c2 - c1
    "encaps": [(5, 4), (6, 5)],  # ctX, ssX
    "decaps": (10, 9),         # c2 - c1
}

# expAM1b 투영 (검증 대상)
AM1B = {"mul_calls": 1287, "sqr_calls": 1274,
        "mul_unit": 112.11, "sqr_unit": 100.12,
        "x25519": 357475}
...
def _med(reps, offset, base=BASE):
...
def _x25519_span(reps, op):
    spec = AN[op]
    if isinstance(spec, list):
        return sum(_med(reps, hi, AN_BASE) - _med(reps, lo, AN_BASE) for hi, lo in spec)
    hi, lo = spec
    return _med(reps, hi, AN_BASE) - _med(reps, lo, AN_BASE)
...
def summarize(log, fails, reps, elf, srec, readback_hash, readback_bytes) -> dict:
    ...
    for op in OPS:
        ...
        t_off = _med(reps, o + 0)
        t_on = _med(reps, o + 1)
        mul_cyc, mul_cnt = _acc(reps, o + 2)
        sqr_cyc, sqr_cnt = _acc(reps, o + 3)
        x = _x25519_span(reps, op)
        per_op[op] = {
            ...
            "mul_cycles": mul_cyc,
            "mul_calls": mul_cnt,
            "mul_unit": mul_cyc / mul_cnt if mul_cnt else 0.0,
            "sqr_cycles": sqr_cyc,
            "sqr_calls": sqr_cnt,
            "sqr_unit": sqr_cyc / sqr_cnt if sqr_cnt else 0.0,
            "mulsqr_cycles": mul_cyc + sqr_cyc,
            "pct_mul": 100.0 * mul_cyc / x if x else 0.0,
            "pct_sqr": 100.0 * sqr_cyc / x if x else 0.0,
            "pct_mulsqr": 100.0 * (mul_cyc + sqr_cyc) / x if x else 0.0,
            "rest": x - mul_cyc - sqr_cyc,
        }

    kv = _parse_kv(log, "expAR equiv:")
...
def print_effects(payload: dict) -> None:
    a = payload["expAM1b_projection"]
    print("")
    print("=== expAR — X25519 field-op in-context 귀속 ===")
    ...
    print("expAM1b 투영: mul %d회 x %.2f + sqr %d회 x %.2f = %.1f%% of %d"
          % (a["mul_calls"], a["mul_unit"], a["sqr_calls"], a["sqr_unit"],
             100.0 * (a["mul_calls"] * a["mul_unit"] + a["sqr_calls"] * a["sqr_unit"])
             / a["x25519"], a["x25519"]))
    for op in OPS:
        d = payload["per_op"][op]
        n = 2 if op == "encaps" else 1
        print("")
        print("--- %s   X25519 %d회 합 %.0f cyc   (probe %+d)"
              % (op, n, d["x25519_same_elf"], d["probe_cost_total"]))
        print("    mul   %10.0f cyc  %6.2f%%   %6.1f calls (호출당 %.0f -> 1회분 %.1f)"
              % (d["mul_cycles"], d["pct_mul"], d["mul_calls"],
                 d["mul_unit"], d["mul_calls"] / n))
        print("    sqr   %10.0f cyc  %6.2f%%   %6.1f calls (호출당 %.0f -> 1회분 %.1f)"
              % (d["sqr_cycles"], d["pct_sqr"], d["sqr_calls"],
                 d["sqr_unit"], d["sqr_calls"] / n))
        print("    mul+sqr %8.0f cyc  %6.2f%%      나머지 %.0f cyc  %.2f%%"
              % (d["mulsqr_cycles"], d["pct_mulsqr"], d["rest"],
                 100.0 - d["pct_mulsqr"]))


def self_test() -> None:
    assert SLOT_N == 447 and FIRST_SLOT == BASE == 435
...
def main() -> int:
    ...
    print("EXPAR_JSON " + json.dumps(payload, ensure_ascii=False, sort_keys=True))
