...
CELLS = [
    ...
    # expCP: 파트너 범위 3배치 -> 5배치
    ("CP keygen F3", 832), ("CP keygen U3", 835),
    ("CP keygen U3", 838), ("CP keygen F3", 841),
    ("CP keygen F5", 844), ("CP keygen U5", 847),
    ("CP keygen U5", 850), ("CP keygen F5", 853),
]
...
def main():
    ...
    print()
    print("# expCP 셀 분포 (백분위, calib 차감)")
    print(f"{'cell':16s} {'span':9s} {'p05':>9s} {'p25':>9s} {'p50':>9s} {'p75':>9s} {'p95':>9s} {'mean':>9s}")
    for label, slot in CELLS:
        if not label.startswith("CP "):
            continue
        for k, span in enumerate(("total", "fused", "residual")):
            b = (slot - BASE + k) * REP_N
            v = sorted(tail[b:b + REP_N])
            adj = calib if span != "residual" else 0
            q = [v[4] - adj, v[24] - adj, v[49] - adj, v[74] - adj, v[94] - adj]
            mn = sum(v) / len(v) - adj
            print(f"{label:16s} {span:9s} " + " ".join(f"{x:9d}" for x in q) + f" {mn:9.0f}")
    print()
