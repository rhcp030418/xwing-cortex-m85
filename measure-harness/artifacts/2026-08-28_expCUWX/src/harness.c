...
static uint8_t dtcm_stack[14336] __attribute__((aligned(8), section(".dtcm")));   /* ML-KEM m4fspeed ?ㅽ깮 ??KB */
...
static void bench_expaf_path_reps(void)
{
    ...
    for (uint32_t r = 0; r < REP_N; r++) {
        ...
        else if (mode == 4u) x25519_u_bench_fused4s(expaf_state);
        else if (mode == 5u) x25519_u_bench_fused4t(expaf_state);
...
#include "expCO_slots.inc"
#include "expCP_slots.inc"
#include "expCR_slots.inc"
#include "expCS_slots.inc"
#include "expCU_slots.inc"
#include "expCW_slots.inc"
#include "expCX_slots.inc"
