...
#include "../mlkem/ap_probe.h"

/* 1단계 게이트: 연산별 Keccak 순열 호출 횟수 집계용 (harness가 리셋/판독).
 * 오버헤드 ~5cyc/호출(순열 5.5k cyc의 0.1%) — 리포트에 각주. */
volatile uint32_t g_keccak_perm_calls;

/* expAO: in-context permutation cycles.  prof_enable=0 이면 기존과 같은 경로다.
 * DWT CYCCNT 직접 판독 (0xE0001004) — expZ prof 계보와 같은 방식. */
volatile uint32_t g_keccak_perm_cyc;
volatile uint32_t g_keccak_prof_enable;
#define EXPAO_CYCCNT (*(volatile uint32_t *) 0xE0001004u)
...
#define KeccakF1600_StatePermute KeccakF1600_StatePermute_counted
 ...
 **************************************************/
static void ap_inner_shake128_squeezeblocks(uint8_t *output, size_t nblocks, shake128ctx *state)
{
#ifdef PROFILE_HASHING
  uint64_t t0 = hal_get_time();
#endif
...
/* ---- expAQ 계측 wrapper (matacc_asm.S 의 bl 도 이 심볼을 탄다) ---- */
void shake128_squeezeblocks(uint8_t *output, size_t nblocks, shake128ctx *state)
{
    AP_WRAP(AP_B_XOF_SQUEEZE, ap_inner_shake128_squeezeblocks(output, nblocks, state));
}
