...
#define EXPZ_PROFILE_CALL(BUCKET, CALL) do {                    \
    uint32_t _t = expz_prof_enable ? EXPZ_CYCCNT : 0u;         \
    CALL;                                                       \
    if (expz_prof_enable) {                                     \
        expz_prof_cyc[BUCKET] += EXPZ_CYCCNT - _t;             \
        expz_prof_calls[BUCKET]++;                              \
    }                                                           \
} while (0)
#define EXPZ_PROFILE_BEGIN() \
    uint32_t _expz_t = expz_prof_enable ? EXPZ_CYCCNT : 0u
#define EXPZ_PROFILE_END(BUCKET) do {                            \
    if (expz_prof_enable) {                                     \
        expz_prof_cyc[BUCKET] += EXPZ_CYCCNT - _expz_t;        \
        expz_prof_calls[BUCKET]++;                              \
    }                                                           \
} while (0)
...
void poly_frombytes_mul_16_32(int32_t *r_tmp, const poly *b, const unsigned char *a) {
    EXPZ_PROFILE_CALL(2, frombytes_mul_asm_16_32(r_tmp, b->coeffs, a, zetas));
}
...
void poly_frombytes_mul_32_32(int32_t *r_tmp, const poly *b, const unsigned char *a) {
    EXPZ_PROFILE_CALL(2, frombytes_mul_asm_acc_32_32(r_tmp, b->coeffs, a, zetas));
}
...
void poly_frombytes_mul_32_16(poly *r, const poly* b, const unsigned char *a, const int32_t *r_tmp) {
    EXPZ_PROFILE_CALL(2, frombytes_mul_asm_acc_32_16(r->coeffs, b->coeffs, a, zetas, r_tmp));
}
...
void poly_noise(poly *r, const unsigned char *seed, unsigned char nonce, int add) {
    ...
    EXPZ_PROFILE_CALL(3, cbd(r, buf, add));
}
...
void poly_basemul_opt_16_32(int32_t *r_tmp, const poly *a, const poly *b, const poly *a_prime) {
    EXPZ_PROFILE_CALL(1, basemul_asm_opt_16_32(r_tmp, a->coeffs, b->coeffs, a_prime->coeffs));
}
...
void poly_basemul_acc_opt_32_32(int32_t *r_tmp, const poly *a, const poly *b, const poly *a_prime) {
    EXPZ_PROFILE_CALL(1, basemul_asm_acc_opt_32_32(r_tmp, a->coeffs, b->coeffs, a_prime->coeffs));
}
...
void poly_basemul_acc_opt_32_16(poly *r, const poly *a, const poly *b, const poly *a_prime, const int32_t * r_tmp) {
    EXPZ_PROFILE_CALL(1, basemul_asm_acc_opt_32_16(r->coeffs, a->coeffs, b->coeffs, a_prime->coeffs, r_tmp));
}
