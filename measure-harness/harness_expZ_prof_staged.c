...
/* Experiment Z2 profiler globals, consumed only by the separate profiler build. */
volatile uint32_t expz_prof_enable = 0;
volatile uint32_t expz_prof_cyc[5];
volatile uint32_t expz_prof_calls[5];
volatile uint32_t expz_prof_keccak[5];
static uint64_t expz_prof_total_cyc[3][5];
static uint32_t expz_prof_total_calls[3][5];
static uint32_t expz_prof_total_keccak[3][5];
static uint32_t expz_prof_output_mismatch;
...
static void expz_prof_reset(void)
{
    expz_prof_enable = 0;
    for (unsigned i = 0; i < 5; i++) {
        expz_prof_cyc[i] = 0;
        expz_prof_calls[i] = 0;
        expz_prof_keccak[i] = 0;
    }
}

static void expz_prof_accumulate(unsigned op)
{
    expz_prof_enable = 0;
    for (unsigned i = 0; i < 5; i++) {
        expz_prof_total_cyc[op][i] += expz_prof_cyc[i];
        expz_prof_total_calls[op][i] += expz_prof_calls[i];
        expz_prof_total_keccak[op][i] += expz_prof_keccak[i];
    }
}

static void expz_prof_print_op(const char *name, unsigned op)
{
    uint32_t cyc[5];
    uint32_t calls[5];
    uint32_t perms;
    uint32_t mat_nonkeccak;

    for (unsigned i = 0; i < 5; i++) {
        cyc[i] = (uint32_t) (expz_prof_total_cyc[op][i] / 8u);
        calls[i] = expz_prof_total_calls[op][i] / 8u;
    }
    perms = expz_prof_total_keccak[op][0] / 8u;
    mat_nonkeccak = cyc[0] > perms * 5526u ? cyc[0] - perms * 5526u : 0u;

    uart_puts("expZ2 "); uart_puts(name);
    uart_puts(": matacc-asm="); uart_put_u32(cyc[0]);
    uart_puts(" matacc-perms="); uart_put_u32(perms);
    uart_puts(" matacc-nonkeccak="); uart_put_u32(mat_nonkeccak);
    uart_puts(" basemul="); uart_put_u32(cyc[1]);
    uart_puts(" frombytes-mul="); uart_put_u32(cyc[2]);
    uart_puts(" cbd="); uart_put_u32(cyc[3]);
    uart_puts(" packing="); uart_put_u32(cyc[4]);
    uart_puts(" C8="); uart_put_u32(mat_nonkeccak + cyc[1] + cyc[2]);
    uart_puts(" C9="); uart_put_u32(cyc[3] + cyc[4]);
    uart_puts(" calls=");
    for (unsigned i = 0; i < 5; i++) {
        if (i) uart_putc('/');
        uart_put_u32(calls[i]);
    }
    uart_puts("\n");
}

static void expz_nonkeccak_profile(void)
{
    uint8_t ref_pk[CRYPTO_PUBLICKEYBYTES];
    uint8_t ref_sk[CRYPTO_SECRETKEYBYTES];
    uint8_t ref_ct[CRYPTO_CIPHERTEXTBYTES];
    uint8_t ref_ss_e[CRYPTO_BYTES];
    uint8_t ref_ss_d[CRYPTO_BYTES];

    memset(expz_prof_total_cyc, 0, sizeof(expz_prof_total_cyc));
    memset(expz_prof_total_calls, 0, sizeof(expz_prof_total_calls));
    memset(expz_prof_total_keccak, 0, sizeof(expz_prof_total_keccak));
    expz_prof_output_mismatch = 0;
    mlkem_ntt_mode = 1;
    mlkem_invntt_mode = 1;

    for (uint32_t seed = 0; seed < 8u; seed++) {
        uint32_t rng_seed = 0x7a000000u + seed;

        PQCLEAN_randombytes_reset(rng_seed);
        expz_prof_enable = 0;
        crypto_kem_keypair(ref_pk, ref_sk);
        crypto_kem_enc(ref_ct, ref_ss_e, ref_pk);
        crypto_kem_dec(ref_ss_d, ref_ct, ref_sk);

        PQCLEAN_randombytes_reset(rng_seed);
        expz_prof_reset(); expz_prof_enable = 1;
        crypto_kem_keypair(mk_pk, mk_sk);
        expz_prof_accumulate(0);
        expz_prof_output_mismatch += byte_mismatches(mk_pk, ref_pk, sizeof(ref_pk));
        expz_prof_output_mismatch += byte_mismatches(mk_sk, ref_sk, sizeof(ref_sk));

        expz_prof_reset(); expz_prof_enable = 1;
        crypto_kem_enc(mk_ct, mk_ss_e, mk_pk);
        expz_prof_accumulate(1);
        expz_prof_output_mismatch += byte_mismatches(mk_ct, ref_ct, sizeof(ref_ct));
        expz_prof_output_mismatch += byte_mismatches(mk_ss_e, ref_ss_e, sizeof(ref_ss_e));

        expz_prof_reset(); expz_prof_enable = 1;
        crypto_kem_dec(mk_ss_d, mk_ct, mk_sk);
        expz_prof_accumulate(2);
        expz_prof_output_mismatch += byte_mismatches(mk_ss_d, ref_ss_d, sizeof(ref_ss_d));
    }
    expz_prof_enable = 0;

    uart_puts("expZ2 profiler: seeds=8 byte-mm=");
    uart_put_u32(expz_prof_output_mismatch);
    uart_puts(" keccak-cyc=5526\n");
    expz_prof_print_op("keygen", 0);
    expz_prof_print_op("encaps", 1);
    expz_prof_print_op("decaps", 2);
}
...
void harness_main(void)
{
    ...
    run_on_stack(expz_nonkeccak_profile, dtcm_stack + sizeof(dtcm_stack));
    harness_fails += expz_prof_output_mismatch;
    ...
    uart_puts("rep arrays ready x100 (169 slots)\n");
