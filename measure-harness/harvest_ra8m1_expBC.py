"""Flash expBC (AT-3), summarize slots 502..513 (full X-Wing with X06 fixed-base)."""
...
def summarize(log, fails, reps, elf, srec, readback_hash, readback_bytes) -> dict:
    ...
    # X02 는 세 연산 모두에 적용 지점이 있다(sqr 은 ladder 전체에서 쓰인다).
    # 따라서 X06(AT-3)의 "decaps 는 ±0.3% 이내" 통제 조건은 여기 적용되지 않는다.
    no_regress = all(res[op]["saving_pct_conservative"] >= 0.0 for op in OPS)
    decaps_ok = True
...
def print_effects(p: dict) -> None:
    print("")
    print("=== expBC (AT-3) — X06 를 넣은 full X-Wing A/B ===")
    ...
    print("")
    print("  gate: 세 연산 모두 회귀 없음 = %s"
          % ("PASS" if p["gate_no_regression"] else "FAIL"))


def self_test() -> None:
    assert SLOT_N == 566 and FIRST_SLOT == BASE == 554
...
def main() -> int:
    ...
    print("EXPBC_JSON " + json.dumps(payload, ensure_ascii=False, sort_keys=True))
