"""Flash expAO, collect the RAM log, and summarize slots 376..389.

In-context Keccak permutation cycles inside X-Wing.  Derived from
...
# per-op offsets: total_off, total_on, keccak_cyc, keccak_calls
OFF = {"keygen": (0, 1, 2, 3), "encaps": (4, 5, 6, 7), "decaps": (8, 9, 10, 11)}
COMBINER = (12, 13)

# expAN slots live in the SAME ELF (362..375), so the ML-KEM bucket used for the
# share arithmetic is read from this build too - no cross-build mixing.
AN_BASE = 362
AN_MLKEM = {"keygen": (1, None), "encaps": (4, None), "decaps": (9, None)}
AN_TOTAL = {"keygen": 0, "encaps": 3, "decaps": 8}
KECCAK_STANDALONE = 5504  # same-ELF [keccak-f1600] slot, for the projection contrast
...
def _stat(reps: list, offset: int, base: int = BASE) -> dict:
...
def summarize(
    ...
    per_op = {}
    for op in OPS:
        o_off, o_on, o_cyc, o_calls = OFF[op]
        t_off = _stat(reps, o_off)["median"]
        t_on = _stat(reps, o_on)["median"]
        kc = _stat(reps, o_cyc)["median"]
        calls = _stat(reps, o_calls)["median"]
        an_mlkem_off = AN_MLKEM[op][0]
        mlkem = _stat(reps, an_mlkem_off, AN_BASE)["median"]
        an_total = _stat(reps, AN_TOTAL[op], AN_BASE)["median"]
        per_op[op] = {
            "total_prof_off": t_off,
            "total_prof_on": t_on,
            "probe_cost_total": t_on - t_off,
            "probe_cost_per_call": (t_on - t_off) / calls if calls else 0.0,
            "keccak_cycles": kc,
            "keccak_calls": calls,
            "keccak_calls_min": _stat(reps, o_calls)["min"],
            "keccak_calls_max": _stat(reps, o_calls)["max"],
            "keccak_cycles_min": _stat(reps, o_cyc)["min"],
            "keccak_cycles_max": _stat(reps, o_cyc)["max"],
            "mlkem_same_elf": mlkem,
            "xwing_total_same_elf": an_total,
            "keccak_cyc_per_call": kc / calls if calls else 0.0,
            "projection_cycles": calls * KECCAK_STANDALONE,
            "measured_vs_projection_pct": (
                100.0 * (kc - calls * KECCAK_STANDALONE) / (calls * KECCAK_STANDALONE)
                if calls else 0.0
            ),
            "pct_of_mlkem": 100.0 * kc / mlkem,
            "pct_of_xwing_total": 100.0 * kc / an_total,
            "mlkem_non_keccak": mlkem - kc,
        }

    comb_total = _stat(reps, COMBINER[0])["median"]
    comb_kc = _stat(reps, COMBINER[1])["median"]
    combiner = {
        "total": comb_total,
        "keccak_cycles": comb_kc,
        "glue": comb_total - comb_kc,
    }

    kv = _parse_kv(log, "expAO equiv:")
    ...
    return {
        ...
        "kat_all_pass": kat,
        "correctness_pass": ok,
        "keccak_standalone_reference": KECCAK_STANDALONE,
        "per_op": per_op,
        "combiner": combiner,
...
def print_effects(payload: dict) -> None:
    print("")
    print("=== expAO — X-Wing 내 Keccak permutation in-context 실측 ===")
    ...
    print("단독 [keccak-f1600] 참조값: %s cyc" % payload["keccak_standalone_reference"])
    print("")
    for op in OPS:
        d = payload["per_op"][op]
        print("--- %s" % op)
        print("    total prof-off / prof-on            %12s / %s  (probe %+d, %.2f cyc/call)"
              % (d["total_prof_off"], d["total_prof_on"],
                 d["probe_cost_total"], d["probe_cost_per_call"]))
        print("    Keccak 누산 (실측)                  %12s cyc  / %d calls = %.1f cyc/call"
              % (d["keccak_cycles"], d["keccak_calls"], d["keccak_cyc_per_call"]))
        print("    같은 calls 의 투영 (calls x 5504)   %12s cyc  → 실측이 %+.2f%%"
              % (d["projection_cycles"], d["measured_vs_projection_pct"]))
        print("    calls 산포 %d~%d (rejection sampling 변동),  Keccak cyc 산포 %s~%s"
              % (d["keccak_calls_min"], d["keccak_calls_max"],
                 d["keccak_cycles_min"], d["keccak_cycles_max"]))
        print("    같은 ELF expAN 버킷: ML-KEM %s / X-Wing total %s"
              % (d["mlkem_same_elf"], d["xwing_total_same_elf"]))
        print("    ML-KEM 중 비중 %6.2f%%   X-Wing 총합 중 %6.2f%%   비-Keccak 잔여 %s cyc"
              % (d["pct_of_mlkem"], d["pct_of_xwing_total"], d["mlkem_non_keccak"]))
        print("")
    c = payload["combiner"]
    print("--- combiner 단독: total %s = permutation %s + glue %s"
          % (c["total"], c["keccak_cycles"], c["glue"]))


def self_test() -> None:
    assert SLOT_N == 390 and FIRST_SLOT == BASE == 376
    used = set()
    for t in OFF.values():
        used |= set(t)
    used |= set(COMBINER)
    assert used == set(range(14)), sorted(used)
    print("self-test OK")
...
def main() -> int:
    ...
    print("EXPAO_JSON " + json.dumps(payload, ensure_ascii=False, sort_keys=True))
