"""Flash expAM1, collect the RAM log, and summarize slots 333..347.
...
def summarize(
    ...
    raw = {}
    for slot in list(SLOT_NAMES) + list(MICRO):
...
def self_test() -> None:
    assert SLOT_N == 348
    assert set(SLOT_NAMES) | set(MICRO) == set(range(333, 348))
    assert BATCH == 64
    print("self-test OK")
...
def main() -> int:
    ...
    print("EXPAM1_JSON " + json.dumps(payload, ensure_ascii=False, sort_keys=True))
