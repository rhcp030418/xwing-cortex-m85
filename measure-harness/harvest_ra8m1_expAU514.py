...
def summarize(log, fails, reps, elf, srec, readback_hash, readback_bytes) -> dict:
    ...
    cts = {k: _stat(reps, o) for k, o in
           (("A_low", 4), ("A_high", 5), ("B_low", 6), ("B_high", 7))}
    ct = {k: v["median"] for k, v in cts.items()}
    for k, v in cts.items():
        ct[k + "_min"] = v["min"]
        ct[k + "_max"] = v["max"]
    ct["B_overlap"] = not (cts["B_low"]["max"] < cts["B_high"]["min"]
                           or cts["B_high"]["max"] < cts["B_low"]["min"])
...
def print_effects(p: dict) -> None:
    ...
    print("    B 분포  low  min/med/max = %d/%d/%d" % (c["B_low_min"], c["B_low"], c["B_low_max"]))
    print("    B 분포  high min/med/max = %d/%d/%d" % (c["B_high_min"], c["B_high"], c["B_high_max"]))
    print("    두 분포 겹침: %s" % ("예 (중앙값 차는 잡음)" if c["B_overlap"] else "아니오 (실제 차이)"))


def self_test() -> None:
    assert SLOT_N == 514 and FIRST_SLOT == BASE == 489
    print("self-test OK")
