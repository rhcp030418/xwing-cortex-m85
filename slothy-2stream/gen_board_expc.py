# 실험 C 보드 커널 생성: fiat 필드 곱 × round6 — 순차 / SLOTHY (2026-08-16)
# 사용법(Windows): py gen_board_expc.py
import os

D = r"c:\Users\cnscj\Dropbox\박도윤개인과제\A주제\slothy-2stream"
OUT = r"C:\Users\cnscj\e2_studio\workspace\blinky\src\gen\expc_board.s"
...
def emit(name, body):
    # fiat 시그니처: r0=out[10], r1=a[10], r2=b[10]. 본문이 전 레지스터 사용 →
    # 카운터 [r12,#376](g_spill[94], 스필 75슬롯=0..299와 분리), 포인터 매 iter 재로드.
...
body_seq = read_body(D + r"\exp_c.s")
out = hdr + emit("expc_seq", body_seq)

opt = D + r"\exp_c_opt.s"
if os.path.exists(opt):
    body_opt = read_body(opt)
    print(f"opt body: {len(body_opt)} instrs")
    out += emit("expc_slothy", body_opt)
else:
    print("exp_c_opt.s 없음 — 솔버 완료 후 재실행")
    out += [".global expc_slothy", ".thumb_func", "expc_slothy:", "\tbx lr", ""]

open(OUT, "w", encoding="utf-8", newline="\n").write("\n".join(out) + "\n")
print(f"seq={len(body_seq)} -> expc_board.s")
