...
volatile uint32_t rep_cyc[63][REP_N];
...
static void bench_keygen_reps(void)
{
    for (uint32_t r = 0; r < REP_N; r++) {
        dwt_span_begin();
        crypto_kem_keypair(mk_pk, mk_sk);
        rep_cyc[3][r] = dwt_span_end();
    }
}

static void bench_encaps_reps(void)
{
    for (uint32_t r = 0; r < REP_N; r++) {
        dwt_span_begin();
        crypto_kem_enc(mk_ct, mk_ss_e, mk_pk);
        rep_cyc[4][r] = dwt_span_end();
    }
}

static void bench_decaps_reps(void)
{
    for (uint32_t r = 0; r < REP_N; r++) {
        dwt_span_begin();
        crypto_kem_dec(mk_ss_d, mk_ct, mk_sk);
        rep_cyc[5][r] = dwt_span_end();
    }
}
...
static void bench_stitch_reps(void)
{
    ...
    /* 실험 J 검증: seq/stitch 각각 곱(fiat_ref)·라운드(C ref) 둘 다 정답이어야 */
    {
        ...
        for (uint32_t v = 0; v < 2; v++) {
            ...
            if (v == 0) expj_seq(1); else expj_stitch(1);
    ...
    /* 실험 M 검증: 곱 정답(현 g_fc_a/b 기준) + 라운드 1회 상태 정답, 두 배치 모두 */
    {
        ...
        for (uint32_t v = 0; v < 2; v++) {
            ...
            if (v == 0) expm_seq(1); else expm_stitch(1);
            for (uint32_t i = 0; i < 10; i++) mm += (g_fc_out[i] != fref[i]);
            for (uint32_t s = 0; s < 4; s++)
                for (uint32_t i = 0; i < 25; i++)
                    mm += (bitdeinterleave(g_mve[4 * i + s], g_mve[128 + 4 * i + s]) != ref[s][i]);
...
void harness_main(void)
{
    ...
    uart_puts("\n");
    run_on_stack(bench_keygen_reps, dtcm_stack + sizeof(dtcm_stack));
    run_on_stack(bench_encaps_reps, dtcm_stack + sizeof(dtcm_stack));
    run_on_stack(bench_decaps_reps, dtcm_stack + sizeof(dtcm_stack));
    ...
    uart_puts("\n");
    harness_fails += expl_mismatch + expm_mismatch;
    ...
    uart_puts("rep arrays ready x100 (17 slots)\n");
